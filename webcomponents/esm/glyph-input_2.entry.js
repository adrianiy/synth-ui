import { r as registerInstance, c as createEvent$1, h } from './index-13b2f165.js';
import { I as Icon } from './icons-f589b4ea.js';
import { F as Flex } from './layout-9e5d99c7.js';
import { s as state } from './context.store-e935f3f6.js';
import { c as cls$1 } from './utils-fe418092.js';
...
const InputComponent = class {
  constructor(hostRef) {
    ...
    this.inputChange = createEvent$1(this, "inputChange", 7);
    this.enterKey = createEvent$1(this, "enterKey", 7);
    /** Input type */
    this.inputType = 'text';
    /** Password visibility */
    ...
    this._renderNumberControls = () => {
      return (h(Flex, { class: "number-control__container" }, h(Icon, { icon: "add", class: cls$1({ disabled: this.value === +(this.max || Infinity) }), onClick: this._increaseValue }), h(Icon, { icon: "remove", class: cls$1({ disabled: this.value === +(this.min || -Infinity) }), onClick: this._decreaseValue })));
    };
    this._renderRangeThumb = () => {
      return (h("div", { class: cls$1('input--range__thumb', {
  ...
  render() {
    return (h(Flex, { row: true, spaced: true, middle: true, style: { '--knob-size': `${this._knobSize}px` }, class: cls$1('input__container', this.interface, {
        search: this.search,
        box: this.box,
        error: this.error,
        disabled: this.disabled,
      }) }, h(Flex, { row: true, middle: true, class: "input__wrapper" }, this.search && (h("glyph-icon", { icon: "search", class: "search", material: this.interface !== "redesign" /* redesign */ })), this.inputType === 'range' && this._renderRangeThumb(), h("input", { ref: ref => (this.ref = ref), type: this._checkInputType(), placeholder: this.placeholder, value: this.value, onKeyUp: this._handleKeyUp, onInput: this._handleInputChange, onChange: this._handleInputOnChange, disabled: this.disabled, min: this.min, max: this.max }), this.inputType === 'number' && this._renderNumberControls()), this.inputType === 'password' && (h(Icon, { icon: `visibility${this.passVisible ? '_off' : ''}`, class: "eye", onClick: this._togglePassVisibility }))));
  }
...
const Scroll = (props, children) => {
  ...
  return (h("div", { style: { '--scrollbar-height': height && `${height}px`, '--scrollbar-width': width && `${width}px` }, class: "scrollbar__container" },
    h("div", { class: cls$1(className, !horizontal && 'scrollbar--hide--x', !vertical && 'scrollbar--hide--y', hideScrollBar && 'scrollbar--hide', tiny && 'scrollbar--tiny'), ref: el => _initializeScroll(el) }, children)));
};
...
const ScrollComponent = class {
  ...
  render() {
    return (h(Scroll, { hideScrollBar: this.hideScrollBar, tiny: this.tiny, horizontal: this.horizontal, vertical: this.vertical, height: this.height, width: this.width, scrollSpeed: this.scrollSpeed, class: cls$1('scrollbar__wrapper', this.containerClass), initCallback: this._initScrollbar }, h("slot", null)));
  }
};
ScrollComponent.style = scrollCss;

export { InputComponent as glyph_input, ScrollComponent as glyph_scroll };
