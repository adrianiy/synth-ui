import { g as getAssetPath } from './index-13b2f165.js';
...
const getComponentClosestLanguage = (element) => {
  const closestElement = element.closest('[lang]');
  return closestElement != null ? closestElement.lang : 'es';
};
const _fetchLocaleStringsForComponent = async (componentName, locale, basePath = '..') => {
  try {
...
const getComponentLocale = (element, languages) => {
  const language = getComponentClosestLanguage(element);
  return languages[language];
};
async function getLocaleComponentStrings(requiredI18n, element, basePath, locale) {
  const componentLanguage = locale || getComponentClosestLanguage(element);
  try {
    const results = await Promise.all(requiredI18n.map(req => _fetchLocaleStringsForComponent(req, componentLanguage, basePath)));
...
const numeralFormat = (value, format, sign = false, negativeSign = true, showZero = false) => {
  let upper = false;
  let lower = false;
...
export { getComponentLocale as a, createCommonjsModule as b, cls as c, commonjsGlobal as d, commonjsRequire as e, getLocaleComponentStrings as f, getFormatedValues as g, getComponentClosestLanguage as h };
