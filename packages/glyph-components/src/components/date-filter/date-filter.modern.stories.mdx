...
export const Template = args => html`<div style="width: 400px; height: 550px"><glyph-date-filter ...=${spread(args)} interface="modern" @dateSelection=${optionClickEvent} @clearEvent=${clearEvent} /></div>`;

<Meta title="Components/Date filter/Modern" parameters={{ viewMode: 'docs' }} />
...
export const optionClickEvent = event => {
    const { detail: { startDate, endDate, description, isDefault } } = event;
...
export const comparableEvent = event => {
    const { detail: { startDate, endDate } } = event;
    console.log(startDate, endDate);
    event.target.comparableStartDate = startDate;
    event.target.comparableEndDate = endDate;
}
export const comparableChange = event => {
    const { detail: compType } = event;
    event.target.comparableType = compType;
}
