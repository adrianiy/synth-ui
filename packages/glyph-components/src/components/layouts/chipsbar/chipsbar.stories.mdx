...
export const Template = args => html`<div style="min-height: 200px"><glyph-chipsbar .filtersConfig=${args.filtersConfig} interface='classic' @filterSelect=${optionClickEvent} @dateSelection=${dateSelect} @comparableDateSelection=${compDateSelect} @comparableChange=${comparableChange} @filterClear=${clearEvent} @updateFilter=${multiSelectEvent} @clearAll=${clearAllEvent} /></div>`;

<Meta title="Layout/Chipsbar/Classic" parameters={{ viewMode: 'docs' }} />
...
export const dateSelect = (event) => {
    const { startDate, endDate, description, isDefault } = event.detail;
    const selected = [ { startDate, endDate, isDefault }];
    const filtersConfig = event.target.filtersConfig;
    const date = filtersConfig.date;
    event.target.filtersConfig = {
        ...filtersConfig,
        date: { ...date, selected, description }
    }
    return null;
}

export const compDateSelect = (event) => {
    const { startDate, endDate } = event.detail;
    const compDates = [ { startDate, endDate }];
    const filtersConfig = event.target.filtersConfig;
    const date = filtersConfig.date;
    event.target.filtersConfig = {
        ...filtersConfig,
...
export const comparableChange = (event) => {
    const comparableType = event.detail;
    const filtersConfig = event.target.filtersConfig;
    const date = filtersConfig.date;
    event.target.filtersConfig = {
        ...filtersConfig,
        date: { ...date, comparableType }
    }
}
...
export const clearEvent = (event) => {
    ...
    event.target.filtersConfig = {...event.target.filtersConfig, ...filter}
}
