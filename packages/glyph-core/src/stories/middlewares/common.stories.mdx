...
- **endpoints**: An object with a boolean value or a path string for each entity you want to consult. i.e: /api/v1/entities/section

> If you don't need some entity as filter you can pass a `false` value in entity configuration.

> If you pass a path string it will override global endpont value
...
- families: /api/v1/entities/families
- subfamilies: /api/v1/entities/subfamilies
...
Take this as an implementation example in **AMIGA FWK** ;)

<div class="warning">

> You'll need to finish pipe execution with a `sendDataBody` to send results back to client.

</div>

```typescript
import { pipe, sendDataBody } from "amiga-services-core";
...
...
router()
  .get("/filters/config", pipe(fetchFiltersConfig({ baseUrl, auth, endpoints }), sendDataBody()))
```
