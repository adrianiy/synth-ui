...
> Date filter must have an option with `active` or `isDefault` attribute as true
...
export const dateRanges = [
    ...
    { description: undefined, startDate: undefined, endDate: undefined } // we'll use this object as custom date selection state
];
...
You can use **GLYPH PIPES** to get filter entities data! take a look at the [DOCS](https://adrianiy.github.io/glyph-ui/core/index.html?path=/docs/glyph-pipes-common-filters--filters)
...
You can set some filters as selected by default using [initialFilters](https://github.com/adrianiy/glyph-ui/blob/ed9b2ba90e3678557132b129987b052bb320f5a7/packages/glyph-core/src/models/filters.ts#L91) object.

## Delegate initialization in providers

If you pass `entities` or `filtersConfig` the provider will initialize filters state.
...
const Home = () => {
    const entities = {};// use your favorite entities fetch method
    const initialFilters = {}; // customize your initial filters object

    return (
        <FilterStateProvider entities={entities} filtersConfig={FiltersConfig} screen="glyph-ui" initialFilters={}>
            <Home/>
        </FilterStateProvider>
    );
};
